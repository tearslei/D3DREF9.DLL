...
static DWORD WINAPI Worker(LPVOID){InputRouter::Instance().Install();HomePanel::Instance().Create(g_inst);MacroEngine::Instance().Start();auto start=Clock::now();bool startup=false;MSG msg{};while(!g_stop.load()){while(PeekMessageW(&msg,nullptr,0,0,PM_REMOVE)){if(msg.message==WM_QUIT){g_stop=true;break;}TranslateMessage(&msg);DispatchMessageW(&msg);}if(!startup&&std::chrono::duration_cast<std::chrono::seconds>(Clock::now()-start).count()>=150){Features().ApplyDelayedStartup();startup=true;}SleepMs(5);}HomePanel::Instance().Destroy();InputRouter::Instance().Uninstall();return 0;}
